...
def test_get_pmc_id_sources_falls_back_when_pmc_body_empty():
    manager = FakeSearchPmcPaperManager([
        "",  # PMC esearch empty (the HPC failure mode)
        '{"esearchresult": {"idlist": ["111"]}}',  # PubMed fallback
        (
            '{"linksets": [{"linksetdbs": ['
...
def test_get_pmc_id_sources_falls_back_when_pmc_body_non_json():
    manager = FakeSearchPmcPaperManager([
...
def test_get_pmc_id_sources_returns_empty_when_pmc_and_pubmed_bodies_empty():
    manager = FakeSearchPmcPaperManager(["", ""])
